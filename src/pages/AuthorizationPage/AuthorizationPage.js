...
function AuthorizationPage() {
  ...
  const onSubmit = async (values, { resetForm }) => {
    ...
    const response = await logInCustomer(values).then((user) => user).catch((err) => console.log(err));

    if (response && response.status === 200) {
      resetForm();
      dispatch(setUserTokenAC(response.data.token));
      dispatch(getUser(response.data.customerNo));
    ...
    <div className={styles.authorizationPageWrapper}>
      ...
      <div className={styles.authorizationPageBlocksPageWrapper}>
        ...
        <div className={`${styles.authorizationBlockWrapper} ${styles.signUpBlockWrapper}`}>
          ...
          <Link to="/sign-up"><button type="button" className={styles.signInButton}>Create an account</button></Link>
